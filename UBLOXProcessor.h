...
// - begin() initialises sensor and probes for sensor-native magvar (eranto)
// - update() reads sensor, converts units, computes COG(M) when eranto known
// - setLiveEranto() receives eranto from SignalK subscription
...
enum class MagVarSource : uint8_t {
    ...
    SENSOR   = 1,   // module provides eranto natively
    SIGNALK  = 2    // eranto subscribed from SignalK server
};
...
public:
    ...
    struct GnssDelta {
        ...
        float cog_m_rad   = NAN;   // COG magnetic (computed, NAN if eranto unknown)
        float mag_var_rad = NAN;   // magnetic eranto, NAN if unknown
    ...
    // Magvar (eranto) source
    MagVarSource getMagVarSource()        const { return _magvar_source; }
    bool         needsErantoSubscription() const { return _magvar_source == MagVarSource::SIGNALK; }

    // Called by SignalKBroker when magneticVariation arrives from server
    void setLiveEranto(float rad);
...
private:
    ...
    float        _eranto_rad    = NAN;  // current best eranto value
    ...
    // Probe sensor for native eranto: take N readings and check if mag_dec != 0
    void probeSensorEranto();
